 ...
 */
export async function saveCharaInfo({
	author: { author_id: chara_id, name: chara },
	turn_key: { chat_id },
}) {
 ...
 * */
export const readCharaInfo = async (path = "./info/charaInfo.json") =>
	new Promise(res =>
...
export const getCharaInfo = async (type, target) => {
	...
	const character = charaInfo.find(
		data => data[type].toLowerCase() == target.toLowerCase()
	);
